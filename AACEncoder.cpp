...
int AACEncoder::InputRawData(const uint8_t* pcmData, size_t sz)
{
	...
	auto quo = m_validSize / m_inputSamples;
	auto rem = m_validSize % m_inputSamples;

	for (size_t i = 0; i < quo; i++)
	{
		ret = faacEncEncode(m_encHandle, (int32_t*)(m_bufferPool + i * m_inputSamples),
	...
	m_offset += rem;
	if (rem != 0)
		memmove(m_bufferPool, m_bufferPool + quo * m_inputSamples, rem);
	return 0;
}
